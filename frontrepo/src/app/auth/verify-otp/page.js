        ...
        try {
          ...
          if (raw) {
            ...
            if (reg && reg.tipo) {
              ...
              // After creating the aluno/professor record, request a refreshed token
              try {
                const refresh = await api.fetchWithAuth('/refresh-role', { method: 'POST' });
                if (refresh && refresh.token) {
                  try { localStorage.setItem('token', refresh.token); } catch (e) {}
                  try { if (typeof setToken === 'function') setToken(refresh.token); } catch (e) {}
                  try { await refreshUser(); } catch (e) {}
                }
              } catch (refreshErr) {
                console.warn('Não foi possível reemitir token após registro:', refreshErr && (refreshErr.message || refreshErr));
              }
