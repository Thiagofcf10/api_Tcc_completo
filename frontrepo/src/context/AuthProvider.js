...
export function AuthProvider({ children }) {
        ...
        (async () => {
          try {
            ...
            const basic = res.user || res;
              ...
              // debug: log verification payload and enriched user
              try { console.debug('AuthProvider: verify payload', { res, basic, enriched }); } catch (e) {}
              setUser(enriched);
          } catch (err) {
            // invalid token
            setTokenState(null);
  ...
  async function login(email, password) {
    ...
    if (data.token) {
      ...
      try {
        ...
        const enriched = await enrichUserRole(basic);
        try { console.debug('AuthProvider: login verify', { verifyRes, basic, enriched, returnedUser: data.user }); } catch (e) {}
        ...
        try { console.debug('AuthProvider: login fallback', { dataUser: data.user, enriched }); } catch (e) {}
        setUser(enriched || data.user || null);
      }
    }
  ...
  async function refreshUser() {
    try {
      ...
      const basic = res.user || res;
      const enriched = await enrichUserRole(basic);
      try { console.debug('AuthProvider: refreshUser', { basic, enriched }); } catch (e) {}
      setUser(enriched);
      return enriched;
    } catch (e) {
      return null;
  ...
  async function enrichUserRole(basicUser) {
    ...
    try {
      ...
      if (profJson && Array.isArray(profJson.data)) {
        ...
        if (found) {
          try { console.debug('enrichUserRole: found professor', { basicUserId: basicUser.id, professor: found }); } catch (e) {}
          return { ...basicUser, tipo: 'professor', professorId: found.id };
        }
      }
      ...
      if (alunoJson && Array.isArray(alunoJson.data)) {
        ...
        if (foundA) {
          try { console.debug('enrichUserRole: found aluno', { basicUserId: basicUser.id, aluno: foundA }); } catch (e) {}
          return { ...basicUser, tipo: 'aluno', alunoId: foundA.id };
        }
      }
